...
def update_scatter_graph(entered_site, payload_range):
    # Filtrar el dataframe por el rango de peso seleccionado en el slider
    low, high = payload_range
    mask = (spacex_df['Payload Mass (kg)'] >= low) & (spacex_df['Payload Mass (kg)'] <= high)
    filtered_df = spacex_df[mask]
    
    if entered_site == 'ALL':
        # Si elige todos los sitios, muestra la dispersión general de peso vs éxito coloreado por Booster
        fig = px.scatter(
            filtered_df, 
            x='Payload Mass (kg)', 
            y='class',
        ...
        # Si elige un sitio específico, aplica el filtro extra de ese sitio
